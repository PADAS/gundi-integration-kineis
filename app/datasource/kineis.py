...
SAMPLE_MESSAGE = {
    "deviceMsgUid": 6961472237487197169,
    "providerMsgId": 1481015819604336642,
    "msgType": "FORMAT TEMPL. 31X8-A1 #1",
    "deviceUid": 50116,
    "deviceRef": "205646",
    "modemRef": "205646",
    "msgDatetime": "2026-03-10T20:17:26.84",
    "acqDatetime": "2026-03-10T20:47:32.13",
    "kineisMetadata": {
        "sat": "KIN1A",
        "mod": "LDA2",
        "level": -131.4,
    ...
    "rawData": "208d450000000000ff000000000000bb8d310000000000d555555555555555",
    "bitLength": 248,
    "dopplerLocId": 1693,
    "dopplerRevision": 1,
    "dopplerDatetime": "2026-03-10T20:17:26.901",
    "dopplerAcqDatetime": "2026-03-10T20:48:52.447",
    "dopplerLocLon": 172.51778,
    "dopplerLocLat": -44.35238,
    "dopplerLocAlt": 0.0,
    "dopplerLocErrorRadius": 1547.0,
    "dopplerDeviceFrequency": 401677711.614,
    "dopplerLocClass": "B",
    "dopplerNbMsg": 2,
}
...
def example() -> Tuple[Dict[str, Any], Dict[str, Any]]:
    raw = KineisRawMessage.parse_obj(SAMPLE_MESSAGE)

    normalizer = KineisMessageNormalizer()
    mapper = EarthRangerMapper()

    normalized = normalizer.normalize(raw)
    observation = mapper.build_observation(normalized, subject_id=normalized.device_ref)
    audit_record = mapper.build_message_audit_record(normalized)
...
def test_doppler_message_normalization() -> None:
    raw = KineisRawMessage.parse_obj(SAMPLE_MESSAGE)
    normalizer = KineisMessageNormalizer()

    result = normalizer.normalize(raw)

    assert result.platform == "cls_kineis"
    assert result.external_message_id == "1481015819604336642"
    assert result.idempotency_key == "cls_kineis:1481015819604336642"

    assert result.location_status == "located"
    assert result.location_method == "doppler"
    assert result.best_location_method == "doppler"

    assert result.best_latitude == -44.35238
    assert result.best_longitude == 172.51778
    assert result.best_location_error_m == 1547.0
    assert result.best_location_quality_class == "B"
    assert result.location_confidence == "low"

    assert result.device_ref == "205646"
    assert result.satellite == "KIN1A"
    assert result.snr == 35.2


def test_er_observation_build() -> None:
    raw = KineisRawMessage.parse_obj(SAMPLE_MESSAGE)
    normalizer = KineisMessageNormalizer()
    mapper = EarthRangerMapper()

    normalized = normalizer.normalize(raw)
    obs = mapper.build_observation(normalized, subject_id="205646")

    assert obs is not None
    assert obs.source == "cls_kineis"
    assert obs.subject_id == "205646"
    assert obs.geometry.type == "Point"
    assert obs.geometry.coordinates == [172.51778, -44.35238]
    assert obs.properties["location_method"] == "doppler"
    assert obs.properties["location_quality_class"] == "B"
    assert obs.properties["location_error_m"] == 1547.0


def test_unlocated_message_builds_no_observation() -> None:
    msg = dict(SAMPLE_MESSAGE)
    msg.pop("dopplerLocLat", None)
    msg.pop("dopplerLocLon", None)
    msg.pop("dopplerDatetime", None)

    raw = KineisRawMessage.parse_obj(msg)
    normalizer = KineisMessageNormalizer()
    mapper = EarthRangerMapper()
    ...
    assert normalized.location_status == "unlocated"
    assert obs is None
...
if __name__ == "__main__":
    ...
    normalized_dict, outputs = example()

    print("=== Normalized ===")
    print(json.dumps(normalized_dict, indent=2, default=str))

    print("\n=== Observation ===")
    print(json.dumps(outputs["observation"], indent=2, default=str))

    print("\n=== Audit Record ===")
    print(json.dumps(outputs["audit_record"], indent=2, default=str))